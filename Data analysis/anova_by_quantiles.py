...
def anova_by_quantiles(dataFile, factor, response, numGroups, outputFile):
    ...
    df['group_rank']= pd.qcut(df[factor],
                             q = numGroups, labels = False)

    # ANOVA in response variable for each quintile of factor values 
    formula = '%s ~ C(%s)' % (response, 'group_rank')
    ...
    with open(outputFile, 'a') as f:
        ...
        print("", file=f)
        print(rp.summary_cont(df[response].groupby(df['group_rank'])), file=f)
        print("", file=f)
        print(anova_table(aov_table), file=f)

def main():
    # tests is a dictionary with key = numGroups (int), and value = outputFile (str)
    tests = {2: 'Data analysis/ANOVA_results_halves.txt', 
            5: 'Data analysis/ANOVA_results_quintiles.txt'
            }

    for numGroups, outputFile in tests.items():
        ...
        for dataFile in ['Data analysis/top_journals_articles.csv', 
            ...
            for factor in ['fre', 'ndc', 'pubdate_year', 'sentence_count', 'word_count']:
                anova_by_quantiles(dataFile = dataFile,
                    factor = factor, 
